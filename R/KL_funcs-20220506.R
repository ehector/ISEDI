LDW_meta <- function(X_1, X_2, y_1, y_2, lambda_seq, family){
  ...
  if(family=="gaussian"){
    ...
    lambda_opt <- (sigma2_hat/n_2) * sum(diag(R%*%solve(G1))) / sum(diag(t(hat_beta_2-hat_beta_1)%*%R%*%(hat_beta_2-hat_beta_1)))
    ...
    # Ryan <- sigma2_hat/n_2*min(XX12_eig)*sum(diag(solve(XX2)))/(max(XX12_eig)^2*sum((hat_beta_2 - hat_beta_1)^2))
    # 
    # MSE <- sapply(lambda_seq, function(lambda) sigma2_hat*sum(diag(solve(XX2 + lambda*n_2/n_1*XX1) -
    #                             lambda*n_2/n_1*solve(XX2+lambda*n_2/n_1*XX1)%*%solve(XX2+lambda*n_2/n_1*XX1)%*%XX1)) +
    #   lambda^2 * (beta_2-beta_1)%*%solve(n_1/n_2*solve(XX1)%*%XX2+lambda*diag(p))%*%solve(n_1/n_2*solve(XX1)%*%XX2+lambda*diag(p)) %*%(beta_2-beta_1) )
    # MSE_opt <- sigma2_hat*sum(diag(solve(XX2 + lambda_opt*n_2/n_1*XX1) - 
    #                                 lambda_opt*n_2/n_1*solve(XX2+lambda_opt*n_2/n_1*XX1)%*%solve(XX2+lambda_opt*n_2/n_1*XX1)%*%XX1)) +
    #   lambda_opt^2 * (beta_2-beta_1)%*%solve(n_1/n_2*solve(XX1)%*%XX2+lambda_opt*diag(p))%*%solve(n_1/n_2*solve(XX1)%*%XX2+lambda_opt*diag(p)) %*%(beta_2-beta_1)
  ...
  if(family=="binomial"){
    ...
    lambda_opt <- (n_1^2/n_2)*sum(diag(R%*%solve(v1))) / sum(diag(R%*%solve(v1)%*%t(X_1)%*%Delta%*%t(Delta)%*%X_1%*%solve(v1)))
